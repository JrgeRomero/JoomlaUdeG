...
class SGAntiVirus {
	...
	public static function CheckStandaloneInstallation($access_key)
	{
        ...
        if (!file_exists($antivirus_module) || filesize(dirname(__FILE__).'/antivirus.php') > filesize($antivirus_module) ) 
        {
			if (file_exists($antivirus_module)) unlink($antivirus_module);
        ...
        if (!file_exists($antivirus_config)) 
        {
            ...
            // Check for correct acckess key
            $handle = fopen($antivirus_config, "r");
            $contents = fread($handle, filesize($antivirus_config));
            fclose($handle);
            
            if (stripos($contents, $access_key) === false)
            {
	...
	public static function CheckAnyModule_status($website_url, $access_key)
	{
		$file_webanalyze_htaccess = JPATH_ROOT.'/webanalyze/.htaccess';
		if (file_exists($file_webanalyze_htaccess)) unlink($file_webanalyze_htaccess);
		
		if (!class_exists('EasyRequest')) include_once(dirname(__FILE__).'/EasyRequest.min.php');
        
        $link = $website_url.'/webanalyze/antivirus.php?task=status&access_key='.$access_key.'&answer='.md5(time().'-'.rand(1,1000));
        ...
        if ( isset($msg['answer']) && $msg['answer'] != '' )
        {
            // The answer is correct
            return true;
        }
        else {
            return 'Can\'t start scanner process. Please try again or contact with SiteGuarding.com support.';
            /*
            // Check htaccess
            $file_htaccess = JPATH_ROOT.'/.htaccess';
            if (!file_exists($file_htaccess))
            {
                ...
                // Backup .htaccess
                copy($file_htaccess, $file_htaccess.'.backup');
                ...
                $contents = str_ireplace("RewriteRule .* index.php", "RewriteCond %{REQUEST_URI} !^/webanalyze [NC]\nRewriteRule .* index.php" , $contents);
                
                self::CreateFile($file_htaccess, $contents); 
                ...
                if ( isset($msg['answer']) && $msg['answer'] != '' )
                {
                    ...
                    rename($file_htaccess, $file_htaccess.'.siteguarding');
                    ...
                    if ( isset($msg['answer']) && $msg['answer'] != '' )
                    {
                        ...
                        // Rename back .htaccess
                        rename($file_htaccess.'.siteguarding', $file_htaccess);
                        
                        return 'Can\'t start scanner process. Please try again or contact with SiteGuarding.com support. #2';
                    }
                }              
            }*/
        }
	...
	{
		if (!class_exists('EasyRequest')) if (!class_exists('EasyRequest')) include_once(dirname(__FILE__).'/EasyRequest.min.php');
	...
	public static function SendFilesForAnalyze($access_key, $email_from)
	{
		if (!class_exists('EasyRequest')) include_once(dirname(__FILE__).'/EasyRequest.min.php');
        
        $website_url = str_replace("/administrator/", "/", JURI::base());
        
        $link = $website_url.'/webanalyze/antivirus.php?task=get_malware_files&showcontent=0&access_key='.$access_key.'&email='.$email_from;
        ...
        $msg = (array)json_decode($msg, true);
        if (isset($msg['status']) && $msg['status'] == 'OK') return true;
        else return false;
	}
...
class SGAntiVirus_module
{
	...
	public static function getReport($domain, $access_key, $session_report_key)
	{
		if (!class_exists('EasyRequest')) include_once(dirname(__FILE__).'/EasyRequest.min.php');
			...
			$client = EasyRequest::create('POST', self::$SITEGUARDING_SERVER.'?action=getreport_ver2', array(
				'form_params' => $post_data
			));
			$client->send();
			$result_json = $client->getResponseBody();
