		...
		if ($src_string_array_count > 1)
		{
			...
			else
			{
				...
				else
				{
					...
					';
					// Funcion obsoleta create_function para PHP 7.4
					//$temp_PHP_func = create_function('&$src_vars, &$article, &$Itemid, &$mainframe, &$app, &$document, &$doc, &$database, &$db, &$user', $src_script);
					
					// AJUSTE PHP 7.4
					$temp_PHP_func = function(&$src_vars, &$article, &$Itemid, &$mainframe, &$app, &$document, &$doc, &$database, &$db, &$user) use ($src_script) {return eval($src_script);};
