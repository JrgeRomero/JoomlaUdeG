	...
	public function onUserAuthenticate($credentials, $options, &$response)
	{
		...
		else
		{
			...
			if($respuesta){
				...
				$credentials = array( 'username' => 'siiau', 'password' => 'userbd2015SIIAU');
				$mainframe->login($credentials, $options=array());				
				//$user = JFACTORY::getUser();
				//$user->name = $primernombre;
				$session = JFactory::getSession();
				//$session->set('NAME_USERSIIAU', "".$primernombre."");
				$session->set('NAME_USERSIIAU', "".$nombre."");
				//****PARA GA
				$session->set('CU', "".$CU."");
				$session->set('CARR', "".$carr_o_dep."");
				$session->set('CODE', "".$codigo."");

			}
			else{//	FIN VALIDACION DE ACCESO SIIAU*/
